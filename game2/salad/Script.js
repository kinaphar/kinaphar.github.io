...
window.onload=function() {
	...
	var url="http://kinaphar.jp/game/salad/index.html";
	...
	var Answer = new Array(1,0,1,1,0,  1,0,0,1,0,  0,1,0,1,1,  0,1,1,0,1);		// ❗正解
	var CorrectAnswer = new Array(
		"サラダ", "カレー", "コーンサラダ", "マカロニサラダ", "ピザ",
		"冷しゃぶサラダ", "家系ラーメン", "サラダ油", "大盛りサラダ", "すき鍋",
		"おせち", "野菜サラダ", "ボルシチ", "ジャーサラダ", "ポテトサラダ",
		"ペペロンチーノ", "スパゲティーサラダ", "ごぼうサラダ", "梅酒", "皿だ"
	);
	...
	var P_Maru = "image/maru.png";					//❗○ボタン
		game.preload([P_Maru]);

	var P_Batsu = "image/batsu.png";					//❗×ボタン
	...
	game.onload = function() {					//✅ロードが終わった後にこの関数が呼び出されるので、この関数内にゲームのプログラムを書こう
		...
		time_label.color = 'rgba(255,255,255,1)';		//❗色　RGB+透明度　今回は白
		time_label.moveTo(200, 20);	
		...
		Q_Text.color = 'rgba(255,255,255,1)';		//❗色　RGB+透明度　今回は白
		Q_Text.width=400;							//❗横幅指定　今回画面サイズ400pxなので、width:400pxだと折り返して二行目表示してくれる
		Q_Text.moveTo(110,320);						//❗移動位置指定
		...
		S_AnswerText.width=400;							//✅横幅指定　今回画面サイズ400pxなので、width:400pxだと折り返して二行目表示してくれる
