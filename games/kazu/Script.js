...
window.onload = function () {
	var game = new Game(700, 800);  				//画面サイズを400*500にする。（このサイズだとスマホでも快適なのでおススメ）

	//結果ツイート時にURLを貼るため、このゲームのURLをここに記入
	var url = "https://kinaphar.github.io/games/kazu/";
	...
	//ドーナッツ画像
	var donutImg = "donut.png";						//game.htmlからの相対パス
	game.preload([donutImg]);					//データを読み込んでおく
	...
	game.onload = function () {					//ロードが終わった後にこの関数が呼び出されるので、この関数内にゲームのプログラムを書こう
		...
		S_MAIN.backgroundColor = "White"; 			//S_MAINシーンの背景は白くした

		//ポイント表示テキスト
		var S_Text = new Label(); 					//テキストはLabelクラス
		S_Text.font = "40px Meiryo";				//フォントはメイリオ 40px 
		S_Text.color = 'rgba(1,1,1,1)';		//色　RGB+透明度　今回は白
		...
		//ドーナッツボタン
		var donut = new Sprite(700, 700);				//画像サイズをここに書く。使う予定の画像サイズはプロパティで見ておくこと
		donut.scale(0.2, 0.2);
		donut.moveTo(118, 900);						//ドーナッツボタンの位置
		donut.image = game.assets[donutImg];			//読み込む画像の相対パスを指定。　事前にgame.preloadしてないと呼び出せない
		S_MAIN.addChild(donut);					//S_MAINにこのドーナッツ画像を貼り付ける  

		donut.ontouchend = function () {				//ドーナッツボタンをタッチした（タッチして離した）時にこの中の内容を実行する
			Point++;									//Pointを1増やす
			game.assets[ClickSound].clone().play();		//クリックの音を鳴らす。

			//クリックしたのでドーナッツ画像のｘ位置を戻す
			this.x = -400;							
		...
		game.onenterframe = function () {
			if (State == 0) { 							//State=0のとき、初期セット状態(Pointの状態を０にして)
				donut.x = -200;						//ドーナッツのｘ座標を指定
				donut.y = 100;						//ドーナッツのy座標を指定
				Point = 0;  							//point初期化
			...
			if (State == 3) {							//状態３（Point６以上から）
				donut.x += 10;
				donut.y = 200 + Math.sin(donut.x / 70) * 100; // ｙ座標を振幅100pxのサイン波で移動(Sinは便利なので慣れとくといいよ！)
			}
			if (State == 4) {							//状態４（Point９以上から）　4は初期セット状態（State=4）と移動状態（State=4.1)の2つに状態をわける		
				donut.y = Math.random() * 400;			//ｙ座標の位置をランダムに決定
				State = 4.1;
			}
			...
			if (State == 5) {							//状態５（Point１２以上から）　 ｙ軸が毎フレーム毎に変化する
				donut.x += 20;						//移動します。
				donut.y = Math.random() * 400;			//ｙ座標の位置を枚フレーム毎にランダム決定
			}
			...
			//ゲームオーバー判定
			if (donut.x >= 400) {						//画面端にぞう山画像が行ってしまったら
				game.popScene();					//S_MAINシーンを外す
				game.pushScene(S_END);				//S_ENDシーンを読み込ませる

				//ゲームオーバー後のテキスト表示
				S_GameOverText.text = "GAMEOVER 記録：" + Point + "個";				//テキストに文字表示 
			}
		...
		S_GameOverText.width = 600;							//横幅指定　今回画面サイズ400pxなので、width:400pxだと折り返して二行目表示してくれる
		S_GameOverText.moveTo(30, 30);						//移動位置指定
		...
		S_Retry.scale(2, 2);
		S_Retry.moveTo(100, 600);						//リトライボタンの位置
		...
		S_Tweet.scale(2, 2);
		S_Tweet.moveTo(430, 600);						//リトライボタンの位置
		...
		S_Tweet.ontouchend = function () {				//S_Tweetボタンをタッチした（タッチして離した）時にこの中の内容を実行する
			...
			window.open("http://twitter.com/intent/tweet?text=頑張って" + Point + "個入手した&hashtags=ahoge&url=" + url); //ハッシュタグにahogeタグ付くようにした。
		};
